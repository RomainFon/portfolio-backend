<section>
    ...
    <form method="post" action="{{ route('studies.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        @csrf
        @method('post')
        ...
        <div>
            ...
            <x-text-input id="start_year" name="start_year" type="number" class="mt-1 block w-full" required/>
            <x-input-error class="mt-2" :messages="$errors->get('start_year')" />
        </div>

        <div>
            <x-input-label for="end_year" :value="__('End Year')" />
            <x-text-input id="end_year" name="end_year" type="number" class="mt-1 block w-full" required/>
